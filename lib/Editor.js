...
function Editor (opts) {
  ...
  BaseWidget.call(self, _(editorWidgetOpts.editor)
    .merge({
      focusable: true,
      multiLine: true
    })
...
Editor.highlightClient = require('./highlight/client');
...
Editor.prototype._initHighlighting = function () {
  ...
  self.data.highlight = {ranges: [], revision: 0, bucket: Editor.highlightClient.getBucket()};
...
Editor.prototype._markMatches = function () {
  ...
  if (selection.isSingleLine() && selectionText.match(/^[\w.-]+$/)
   ...
   && (line[selection.end.column] || ' ').match(/\W/)) {
    self.textBuf.scan(new RegExp('\\b'+util.text.escapeRegExp(selectionText)+'\\b', 'g'), function (match) {
...
Editor.prototype._initHandlers = function () {
      ...
      switch (binding) {
        ...
        default:
          if (!binding && !key.ctrl && ch) {
            ...
            if (!self.readOnly()) {
              if (selectionRange.isEmpty() && !self.screen.insertMode() && !enterPressed) selectionRange.end.column++;
  ...
  ['path-changed', 'changed'].forEach(function (evt) {
    self.textBuf.on(evt, function () { self.screen.header.render(); });
  });
...
Editor.prototype._renderableSpace = function (match) {
  return !this.buffer.options.visibleWhiteSpace
    ? match
    : util.markup(_.repeat('\u00b7', match.length).join(''), this.options.style.whiteSpace);
};
...
Editor.prototype._updateContent = function () {
  ...
  util.text.splitLines(blessed.escape(self.textBuf.getTextInRange({
